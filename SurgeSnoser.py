...
ascii_banner = pyfiglet.figlet_format("SURGE SNOSER")
colored_banner = colored(ascii_banner, color='red')  # Êðàñèì â öâåò
print('by @SURGEEBET')
print('questions - @SURGEEBET')
print('VERSION - 1.0.0')
print('PRICE - free version')
print('-------------------------------------------------------------')
...
def send_complaint(username, telegram_id, number, email, repeats, complaint_choice, proxies=None):
	...
	if complaint_choice == "1":
		...
		text = f"Здраствуйте. Аккаунт {username}, id {telegram_id} оскорбляет меня и мою маму. Мне это очень не приятно, поэтому и пишу вам. Огромная просьба разобраться и заблокировать данного пользователя т.к это нарушает политику сервиса. Благодарю"
		...
		text = f"На сервисе telegram обнаружил пользователя который накручивает на канал реакции, подписки и просмотры. Ссылка на посты с накруткой и аккаунт администратора: {username}, id администратора на случай если поменяет юзернейм: {telegram_id}. Просьба разобраться и заблокировать пользователя т.к это нарушает правила Telegram"
	...
	try:
		...
		for _ in range(int(repeats)):
			...
			if response.status_code == 200:
				...
				print(colored(f"The report has been successfully sent", 'red'))

				print(colored(f"From: {email} {number}", 'green'))

			else:

				print("Ошибка:. code:", response.status_code)
...
def complaint():
	...
	print(colored("[1] Virtual number", "red"))
	print(colored("[2] Premium","magenta"))
	print(colored("[3] Offense messages","red"))
	print(colored("[4] Violation of the rules", "magenta"))
	print(colored("[5] Sessions", "red"))
	print(colored("[6] Spam", "magenta"))
	print(colored("[7] Price", "red"))
	print(colored("[8] Cheating followers", "magenta"))
	complaint_choice = input(colored("Choose option: ", "red"))
	if complaint_choice in ["1", "2", "3", "4", "6", "7", "8"]:
		...
		number = input("Enter account phone number: ")
